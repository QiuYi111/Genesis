...
from .services.llm_provider import LLMProvider, NullProvider, TrinityActions, Planner, NullPlanner
from typing import Tuple

class Trinity:
    """World rules generator class - refactored for observe/adjust pattern
    ...
    Attributes:
        ...
        planner: Strategy for planning actions
        _stats: Internal statistics for observation
    """
    def __init__(self, bible: Bible, era_prompt: str, provider: Optional[LLMProvider] = None):
        self.bible = bible
        ...
        self.provider = provider or NullProvider()  # Default to null provider for testing
        self.planner = NullPlanner()  # Start with null planner, can be overridden
        self._stats = {}  # Internal statistics for observation

    def observe(self, events: list[dict]) -> None:
        """Observe simulation events and update internal statistics"""
        # Update internal statistics based on events
        for event in events:
            event_type = event.get('type', 'unknown')
            if event_type not in self._stats:
                self._stats[event_type] = []
            self._stats[event_type].append(event)
        
        logger.debug(f"[Trinity] Observed {len(events)} events, stats: {list(self._stats.keys())}")
    
    def adjust(self, world) -> TrinityActions:
        """Adjust world based on observations and return TrinityActions"""
        # Calculate resource status for decision making
        signals = self._calculate_resource_status(world)
        
        # Use planner to determine actions
        plan = self.planner.plan(signals)
        
        # Create TrinityActions from plan
        actions = TrinityActions(
            resource_regen_multiplier=plan.get("regen", 1.0),
            terrain_adjustments=plan.get("terrain"),
            skill_updates=plan.get("skills")
        )
        
        logger.debug(f"[Trinity] Generated actions: regen={actions.resource_regen_multiplier}")
        return actions
    ...
    def set_planner(self, planner: Planner) -> None:
        """Set the planner strategy for Trinity"""
        self.planner = planner
        logger.info(f"[Trinity] Planner set to {type(planner).__name__}")
    
    def get_stats(self) -> dict:
        """Get internal statistics for testing and monitoring"""
        return self._stats.copy()
    
    def compute_resource_status(self, world) -> dict:
        """Public method to compute resource status - used by adjust()"""
        return self._calculate_resource_status(world)
